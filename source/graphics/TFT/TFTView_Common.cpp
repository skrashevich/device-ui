...
void TFTView_Common::newMessage(uint32_t from, uint32_t to, uint8_t ch, const char *msg, uint32_t &msgTime, bool restore)
{
    ...
    if (!restore) {
        ...
        if (container != THIS->activeMsgContainer || THIS->activePanel != objects.messages_panel) {
            ...
            if (!isMuted) {
                ...
                if (THIS->activePanel != objects.messages_panel && THIS->db.uiConfig.alert_enabled) {
...
void TFTView_Common::showMessages(uint8_t ch)
{
    ...
    lv_label_set_text(objects.top_group_chat_label, lv_label_get_text(THIS->channel[ch]));
...
void TFTView_Common::ui_event_map_url_dropdown(lv_event_t *e)
{
#if defined(VIEW_320x240) || defined(VIEW_240x320)
        ...
        if (!sdCard->isUpdated()) {
            ...
            if (mapStyles.find("/map") != mapStyles.end()) {
                ...
                for (auto it : mapStyles) {
                    ...
                    if (!url.empty()) {
                        TileProvider::addTemplate("URL: " + it, url);
#if defined(VIEW_320x240) || defined(VIEW_240x320)
