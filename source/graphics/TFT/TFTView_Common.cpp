...
#define THIS TFTView_Common::commonInstance
...
// Note: using 'extern const' for external linkage so subclass TUs can reference these
extern const lv_color_t colorRed = LV_COLOR_HEX(0xff5555);
extern const lv_color_t colorDarkRed = LV_COLOR_HEX(0xa70a0a);
extern const lv_color_t colorOrange = LV_COLOR_HEX(0xff8c04);
extern const lv_color_t colorYellow = LV_COLOR_HEX(0xdbd251);
extern const lv_color_t colorBlueGreen = LV_COLOR_HEX(0x05f6cb);
extern const lv_color_t colorBlue = LV_COLOR_HEX(0x436C70);
extern const lv_color_t colorGray = LV_COLOR_HEX(0x757575);
extern const lv_color_t colorLightGray = LV_COLOR_HEX(0xAAFBFF);
extern const lv_color_t colorMidGray = LV_COLOR_HEX(0x808080);
...
void syncVirtualKeyboardLayout(lv_obj_t *keyboard)
{
    ...
    lv_keyboard_set_mode(keyboard, russianLayout ? (upperLayout ? LV_KEYBOARD_MODE_USER_2 : LV_KEYBOARD_MODE_USER_1)
                                                 : (upperLayout ? LV_KEYBOARD_MODE_TEXT_UPPER : LV_KEYBOARD_MODE_TEXT_LOWER));
#endif
}
...
TFTView_Common *TFTView_Common::commonInstance = nullptr;
uint32_t TFTView_Common::currentNode = 0;
lv_obj_t *TFTView_Common::currentPanel = nullptr;
lv_obj_t *TFTView_Common::spinnerButton = nullptr;
time_t TFTView_Common::startTime = 0;
uint32_t TFTView_Common::pinKeys = 0;
bool TFTView_Common::screenLocked = false;
bool TFTView_Common::screenUnlockRequest = false;

static lv_obj_t *symIndicator = nullptr;

TFTView_Common::TFTView_Common(const DisplayDriverConfig *cfg, DisplayDriver *driver)
 ...
 */
uint32_t TFTView_Common::preset2val(meshtastic_Config_LoRaConfig_ModemPreset preset)
{
 ...
 */
meshtastic_Config_LoRaConfig_ModemPreset TFTView_Common::val2preset(uint32_t val)
{
 ...
 */
uint32_t TFTView_Common::role2val(meshtastic_Config_DeviceConfig_Role role)
{
 ...
 */
meshtastic_Config_DeviceConfig_Role TFTView_Common::val2role(uint32_t val)
{
 ...
 */
uint32_t TFTView_Common::language2val(meshtastic_Language lang)
{
    switch (lang) {
 ...
 */
meshtastic_Language TFTView_Common::val2language(uint32_t val)
{
    switch (val) {
    case 0:
...
void TFTView_Common::timer_event_reboot(lv_timer_t *timer)
{
    ILOG_INFO("Rebooting...");
...
void TFTView_Common::timer_event_shutdown(lv_timer_t *timer)
{
    ILOG_INFO("Shutdown...");
    THIS->controller->stop();
    delay(1000);
...
void TFTView_Common::writePacketLog(const meshtastic_MeshPacket &p)
{
...
void TFTView_Common::updateStatistics(const meshtastic_MeshPacket &p)
{
    struct Stats {
        uint32_t id;
        uint16_t row;
    ...
    for (auto it2 : stats) {
        ...
        row++;
        if (row >= statisticTableRows) // fill rows till bottom of display
            break;
    }
}
...
bool TFTView_Common::backupFullConfig(void)
{
...
bool TFTView_Common::restoreFullConfig(void)
{
#if defined(HAS_SDCARD) || defined(HAS_SD_MMC) || defined(ARCH_PORTDUINO)
    std::stringstream path;
    path << "/backup/" << std::hex << std::setw(8) << std::setfill('0') << ownNode << ".cfg";
...
void TFTView_Common::addNode(uint32_t nodeNum, uint8_t ch, const char *userShort, const char *userLong, uint32_t lastHeard,
                             eRole role, bool hasKey, bool unmessagable)
{
    ...
    // virtual hook for view-specific customization (e.g. 320x240 hides extended labels)
    THIS->onAddNodeExtra(p);

    lv_obj_add_event_cb(nodeButton, ui_event_NodeButton, LV_EVENT_ALL, (void *)nodeNum);

    // move node into new position within nodePanel
    if (lastHeard) {
        lv_obj_t **children = THIS->objects.nodes_panel->spec_attr->children;
        int i = THIS->objects.nodes_panel->spec_attr->child_cnt - 1;
        while (i > 1) {
            if (lastHeard <= (time_t)(children[i - 1]->LV_OBJ_IDX(node_lh_idx)->user_data))
                break;
            i--;
        }
        if (i >= 1 && i < THIS->objects.nodes_panel->spec_attr->child_cnt - 1) {
...
void TFTView_Common::addOrUpdateNode(uint32_t nodeNum, uint8_t channel, uint32_t lastHeard, const meshtastic_User &cfg)
{
    if (THIS->nodes.find(nodeNum) == THIS->nodes.end()) {
...
void TFTView_Common::updateNode(uint32_t nodeNum, uint8_t ch, const meshtastic_User &cfg)
{
    THIS->db.user = cfg;
    ...
    if (it != THIS->nodes.end() && it->second) {
        if (it->first == THIS->ownNode) {
            ...
            lv_label_set_text(THIS->objects.basic_settings_user_label, buf);

            char buf1[30], buf2[40];
            lv_dropdown_set_selected(THIS->objects.settings_device_role_dropdown,
                                     THIS->role2val(meshtastic_Config_DeviceConfig_Role(cfg.role)));
            lv_dropdown_get_selected_str(THIS->objects.settings_device_role_dropdown, buf1, sizeof(buf1));
            lv_snprintf(buf2, sizeof(buf2), _("Device Role: %s"), buf1);
            lv_label_set_text(THIS->objects.basic_settings_role_label, buf2);

            // update DB
            strcpy(THIS->db.short_name, cfg.short_name);
            strcpy(THIS->db.long_name, cfg.long_name);
            THIS->db.config.device.role = cfg.role;
        }
        ...
        // update chat name
        auto ct = THIS->chats.find(it->first);
        if (ct != THIS->chats.end()) {
            char buf[64];
...
void TFTView_Common::removeNode(uint32_t nodeNum)
{
    auto it = THIS->nodes.find(nodeNum);
...
void TFTView_Common::purgeNode(uint32_t nodeNum)
{
    if (THIS->nodeCount <= 1)
        return;

    lv_obj_t **children = THIS->objects.nodes_panel->spec_attr->children;
    int last = THIS->objects.nodes_panel->spec_attr->child_cnt - 1;
    int i = last;
    ...
    while ((eRole)(long)(children[i]->LV_OBJ_IDX(node_img_idx)->user_data) != eRole::unknown ||
           ...
           THIS->chats.find((unsigned long)(children[i]->LV_OBJ_IDX(node_lbl_idx)->user_data)) != THIS->chats.end()) {
    ...
    lv_obj_delete(p);
    {
        auto it = THIS->messages.find(oldest);
        if (it != THIS->messages.end()) {
            lv_obj_delete(it->second);
            THIS->messages.erase(oldest);
        }
    }

    {
        auto it = THIS->chats.find(oldest);
        if (it != THIS->chats.end()) {
            lv_obj_delete(it->second);
            THIS->chats.erase(oldest);
            updateActiveChats();
        }
    }
    THIS->removeFromMap(oldest);
    THIS->nodes.erase(oldest);
    THIS->nodeCount--;
    THIS->nodesChanged = true; // flag to force re-apply node filter
}

bool TFTView_Common::applyNodesFilter(uint32_t nodeNum, bool reset)
{
    lv_obj_t *panel = THIS->nodes[nodeNum];
    bool hide = false;
    if (nodeNum != THIS->ownNode /* && filter.active*/) { // TODO
        if (lv_obj_has_state(THIS->objects.nodes_filter_unknown_switch, LV_STATE_CHECKED)) {
            if (lv_img_get_src(panel->LV_OBJ_IDX(node_img_idx)) == &img_circle_question_image) {
                hide = true;
            }
        }
        if (lv_obj_has_state(THIS->objects.nodes_filter_offline_switch, LV_STATE_CHECKED)) {
            time_t lastHeard = (time_t)panel->LV_OBJ_IDX(node_lh_idx)->user_data;
            if (lastHeard == 0 || THIS->curtime - lastHeard > THIS->secs_until_offline)
                hide = true;
        }
        if (lv_obj_has_state(THIS->objects.nodes_filter_public_key_switch, LV_STATE_CHECKED)) {
            bool hasKey = (unsigned long)panel->LV_OBJ_IDX(node_bat_idx)->user_data == 1;
            if (!hasKey)
                hide = true;
        }
        if (lv_dropdown_get_selected(THIS->objects.nodes_filter_channel_dropdown) != 0) {
            int selected = lv_dropdown_get_selected(THIS->objects.nodes_filter_channel_dropdown);
            if (selected != 0) {
        ...
        if (lv_dropdown_get_selected(THIS->objects.nodes_filter_hops_dropdown) != 0) {
            int32_t hopsAway = (signed long)panel->LV_OBJ_IDX(node_sig_idx)->user_data;
            int selected = lv_dropdown_get_selected(THIS->objects.nodes_filter_hops_dropdown) - 7;
            if (hopsAway < 0)
                hide = true;
            else if (selected <= 0) {
...
#if 0
        if (lv_obj_has_state(THIS->objects.nodes_filter_mqtt_switch, LV_STATE_CHECKED)) {
            bool viaMqtt = false; // TODO (unsigned long)panel->LV_OBJ_IDX(node_sig_idx)->user_data;
            if (viaMqtt)
                hide = true;
        }
#endif
        if (lv_obj_has_state(THIS->objects.nodes_filter_position_switch, LV_STATE_CHECKED)) {
            if (lv_label_get_text(panel->LV_OBJ_IDX(node_pos1_idx))[0] == '\0')
                hide = true;
        }
        const char *name = lv_textarea_get_text(THIS->objects.nodes_filter_name_area);
        if (name[0] != '\0') {
    ...
    if (THIS->map)
        THIS->map->update(nodeNum, hide);

    bool highlight = false;
    if (true /*highlight.active*/) { // TODO
        if (lv_obj_has_state(THIS->objects.nodes_hl_active_chat_switch, LV_STATE_CHECKED)) {
            auto it = THIS->chats.find(nodeNum);
            if (it != THIS->nodes.end()) {
                lv_obj_set_style_border_color(panel, colorOrange, LV_PART_MAIN | LV_STATE_DEFAULT);
                highlight = true;
            }
        }
        if (lv_obj_has_state(THIS->objects.nodes_hl_position_switch, LV_STATE_CHECKED)) {
            if (lv_label_get_text(panel->LV_OBJ_IDX(node_pos1_idx))[0] != '\0') {
                lv_obj_set_style_border_color(panel, colorBlueGreen, LV_PART_MAIN | LV_STATE_DEFAULT);
                highlight = true;
            }
        }
        if (lv_obj_has_state(THIS->objects.nodes_hl_telemetry_switch, LV_STATE_CHECKED)) {
        ...
        if (lv_obj_has_state(THIS->objects.nodes_hliaq_switch, LV_STATE_CHECKED)) {
        ...
        const char *name = lv_textarea_get_text(THIS->objects.nodes_hl_name_area);
        if (name[0] != '\0') {
            if (strcasestr(lv_label_get_text(panel->LV_OBJ_IDX(node_lbl_idx)), name) ||
                strcasestr(lv_label_get_text(panel->LV_OBJ_IDX(node_lbs_idx)), name)) {
...
void TFTView_Common::setNodeImage(uint32_t nodeNum, eRole role, bool unmessagable, lv_obj_t *img)
{
    uint32_t bgColor, fgColor;
    std::tie(bgColor, fgColor) = THIS->nodeColor(nodeNum);
    if (unmessagable) {
...
void TFTView_Common::updateNodesStatus(void)
{
    ...
    lv_label_set_text(THIS->objects.home_nodes_label, buf);

    if (THIS->nodesFiltered)
        lv_snprintf(buf, sizeof(buf), _("Filter: %d of %d nodes"), THIS->nodeCount - THIS->nodesFiltered, THIS->nodeCount);
    lv_label_set_text(THIS->objects.top_nodes_online_label, buf);
}

void TFTView_Common::updateNodesFiltered(bool reset)
{
    static auto it = THIS->nodes.begin();
    if (reset || THIS->nodesChanged) {
        THIS->nodesFiltered = 0;
        THIS->nodesChanged = false;
        THIS->processingFilter = true;
        it = THIS->nodes.begin();
    }

    for (int i = 0; i < 10 && it != THIS->nodes.end(); i++) {
...
void TFTView_Common::updateLastHeard(uint32_t nodeNum)
{
    auto it = THIS->nodes.find(nodeNum);
    if (it != THIS->nodes.end() && it->second) {
        ...
        if (it->first != THIS->ownNode) {
            ...
            if (lv_group_ll && act)
                _lv_ll_move_before(lv_group_ll, act, _lv_ll_get_next(lv_group_ll, THIS->topNodeLL));
        }
    }
}
...
void TFTView_Common::updateAllLastHeard(void)
{
    ...
    for (auto it : THIS->nodes) {
        char buf[32];
        if (it.first == THIS->ownNode) { // own node is always now, so do update
            lastHeard = THIS->curtime;
...
lv_obj_t *TFTView_Common::newMessageContainer(uint32_t from, uint32_t to, uint8_t ch)
{
    if (to == UINT32_MAX || from == 0) {
        if (THIS->channelGroup[ch] != nullptr)
            return THIS->channelGroup[ch];
    } else {
        auto it = THIS->messages.find(from);
        if (it != THIS->messages.end() && it->second)
            return it->second;
    }

    // create container for new messages
    lv_obj_t *container = lv_obj_create(THIS->objects.messages_panel);
    ...
    lv_obj_clear_flag(container, lv_obj_flag_t(LV_OBJ_FLAG_PRESS_LOCK | LV_OBJ_FLAG_GESTURE_BUBBLE | LV_OBJ_FLAG_SNAPPABLE |
                                               LV_OBJ_FLAG_SCROLL_ELASTIC | LV_OBJ_FLAG_CLICK_FOCUSABLE)); /// Flags
    ...
    if (to == UINT32_MAX || from == 0) {
        THIS->channelGroup[ch] = container;
    } else {
        THIS->messages[from] = container;
    }
...
void TFTView_Common::newMessage(uint32_t from, uint32_t to, uint8_t ch, const char *msg, uint32_t &msgTime, bool restore)
{
    ...
    if (to == UINT32_MAX) { // message for group, prepend short name to msg
        if (THIS->nodes.find(from) == THIS->nodes.end()) {
            ...
            char *userData = (char *)&(THIS->nodes[from]->LV_OBJ_IDX(node_lbs_idx)->user_data);
        ...
        buf[pos++] = ' ';
        container = THIS->channelGroup[ch];
    } else { // message for us
        container = THIS->messages[from];
    }
    ...
    pos += THIS->timestamp(&buf[pos], msgTime, !restore);
    ...
    if (!restore) {
        ...
        if (container != THIS->activeMsgContainer || THIS->activePanel != THIS->objects.messages_panel) {
            // Check if channel is muted - DMs (to specific node, not broadcast) always notify
            bool isDM = (to != 0 && to != 0xFFFFFFFF);
            bool isMuted = !isDM && THIS->db.channel[ch].settings.module_settings.is_muted;
            if (!isMuted) {
                THIS->unreadMessages++;
                updateUnreadMessages();
                if (THIS->activePanel != THIS->objects.messages_panel && THIS->db.uiConfig.alert_enabled) {
                    showMessagePopup(from, to, ch, lv_label_get_text(THIS->nodes[from]->LV_OBJ_IDX(node_lbl_idx)));
                }
            }
            lv_obj_add_flag(container, LV_OBJ_FLAG_HIDDEN);
            // virtual hook for view-specific actions (e.g. badge updates)
            THIS->onNewMessageExtra(from, isDM);
        }
        if (container != THIS->activeMsgContainer)
...
void TFTView_Common::newMessage(uint32_t nodeNum, lv_obj_t *container, uint8_t ch, const char *msg)
{
    ...
    if (THIS->state == MeshtasticView::eRunning) {
        lv_obj_scroll_to_view(hiddenPanel, LV_ANIM_ON);
        lv_obj_move_foreground(THIS->objects.message_input_area);
    }
    lv_obj_add_event_cb(hiddenPanel, ui_event_chatNodeButton, LV_EVENT_CLICKED, (void *)nodeNum);
}

void TFTView_Common::restoreMessage(const LogMessage &msg)
{
    if (msg.from == THIS->ownNode) {
        lv_obj_t *container = nullptr;
        if (msg.to == UINT32_MAX) {
            if (msg.trashFlag && THIS->chats.find(msg.ch) != THIS->chats.end()) {
                ILOG_DEBUG("trashFlag set for channel %d", msg.ch);
                THIS->eraseChat(msg.ch);
                return;
            } else {
                container = newMessageContainer(msg.from, msg.to, msg.ch);
            }
        } else {
            if (THIS->nodes.find(msg.to) != THIS->nodes.end()) {
                if (msg.trashFlag && THIS->chats.find(msg.to) != THIS->chats.end()) {
                    ILOG_DEBUG("trashFlag set for node %08x", msg.to);
                    THIS->eraseChat(msg.to);
                    return;
                } else {
        ...
        if (container) {
            if (container != THIS->activeMsgContainer)
                lv_obj_add_flag(container, LV_OBJ_FLAG_HIDDEN);
            THIS->addMessage(container, msg.time, 0, (char *)msg.bytes, msg.status);
        }
    } else if (THIS->nodes.find(msg.from) != THIS->nodes.end()) {
        if (msg.trashFlag && THIS->chats.find(msg.from) != THIS->chats.end()) {
            ILOG_DEBUG("trashFlag set for node %08x", msg.from);
            THIS->eraseChat(msg.from);
            return;
        } else {
        ...
        uint32_t len = THIS->timestamp(buf + pos, msg.time, false);
...
void TFTView_Common::addChat(uint32_t from, uint32_t to, uint8_t ch)
{
    uint32_t index = ((to == UINT32_MAX || from == 0) ? ch : from);
    auto it = THIS->chats.find(index);
    if (it != THIS->chats.end())
        return;

    lv_obj_t *chatDelBtn = nullptr;
    lv_obj_t *parent_obj = THIS->objects.chats_panel;

    // ChatsButton
    lv_obj_t *chatBtn = lv_btn_create(parent_obj);
    lv_obj_set_pos(chatBtn, 0, 0);
    lv_obj_set_size(chatBtn, LV_PCT(100), THIS->buttonSize);
    ...
    char buf[64];
    if (to == UINT32_MAX || from == 0) {
        sprintf(buf, "%d: %s", (int)ch, lv_label_get_text(THIS->channel[ch]));
    } else {
...

    {
        lv_obj_t *parent_obj = chatBtn;
        {
            ...
            THIS->objects.chats_button_label = obj;
    ...
    THIS->chats[index] = chatBtn;
    updateActiveChats();
    if (index > c_max_channels) {
        if (THIS->nodes.find(index) != THIS->nodes.end())
...
void TFTView_Common::showMessages(uint8_t ch)
{
    if (!THIS->messagesRestored) {
        // display message restoration progress banner
        lv_obj_clear_flag(THIS->objects.msg_popup_panel, LV_OBJ_FLAG_HIDDEN);
        lv_group_focus_obj(THIS->objects.msg_popup_button);
        return;
    }

    lv_obj_add_flag(THIS->activeMsgContainer, LV_OBJ_FLAG_HIDDEN);
    THIS->activeMsgContainer = THIS->channelGroup[ch];
    if (!THIS->activeMsgContainer) {
        THIS->activeMsgContainer = newMessageContainer(0, UINT32_MAX, ch);
    }

    THIS->activeMsgContainer->user_data = (void *)(uint32_t)ch;
    lv_obj_clear_flag(THIS->activeMsgContainer, LV_OBJ_FLAG_HIDDEN);
    lv_label_set_text(THIS->objects.top_group_chat_label, lv_label_get_text(THIS->channel[ch]));
    THIS->ui_set_active(THIS->objects.messages_button, THIS->objects.messages_panel, THIS->objects.top_group_chat_panel);
}

void TFTView_Common::showMessages(uint32_t nodeNum)
{
    lv_obj_add_flag(THIS->activeMsgContainer, LV_OBJ_FLAG_HIDDEN);
    THIS->activeMsgContainer = THIS->messages[nodeNum];
    if (!THIS->activeMsgContainer) {
        THIS->activeMsgContainer = newMessageContainer(nodeNum, 0, 0);
    }
    THIS->activeMsgContainer->user_data = (void *)nodeNum;
    lv_obj_clear_flag(THIS->activeMsgContainer, LV_OBJ_FLAG_HIDDEN);
    lv_obj_t *p = THIS->nodes[nodeNum];
    if (p) {
        lv_label_set_text(THIS->objects.top_messages_node_label, lv_label_get_text(p->LV_OBJ_IDX(node_lbl_idx)));
        THIS->ui_set_active(THIS->objects.messages_button, THIS->objects.messages_panel, THIS->objects.top_messages_panel);
        switch ((unsigned long)p->LV_OBJ_IDX(node_bat_idx)->user_data) {
        case 0:
            lv_obj_set_style_bg_image_src(THIS->objects.top_messages_node_image, &img_lock_channel_image,
                                          LV_PART_MAIN | LV_STATE_DEFAULT);
            break;
        case 1:
            lv_obj_set_style_bg_image_src(THIS->objects.top_messages_node_image, &img_lock_secure_image,
                                          LV_PART_MAIN | LV_STATE_DEFAULT);
            break;
        default:
            lv_obj_set_style_bg_image_src(THIS->objects.top_messages_node_image, &img_lock_slash_image,
                                          LV_PART_MAIN | LV_STATE_DEFAULT);
            break;
        }
        THIS->unreadMessages = 0; // TODO: not all messages may be actually read
        updateUnreadMessages();
    } else {
        // TODO: log error
    }
...
void TFTView_Common::highlightChat(uint32_t from, uint32_t to, uint8_t ch)
{
    uint32_t index = ((to == UINT32_MAX || from == 0) ? ch : from);
    auto it = THIS->chats.find(index);
    if (it != THIS->chats.end()) {
...
void TFTView_Common::updateActiveChats(void)
{
    char buf[48];
    sprintf(buf, _p("%d active chat(s)", THIS->chats.size()), THIS->chats.size());
    lv_label_set_text(THIS->objects.top_chats_label, buf);
}

void TFTView_Common::notifyRestoreMessages(int32_t percentage)
{
    lv_bar_set_value(THIS->objects.message_restore_bar, percentage, LV_ANIM_OFF);
}

void TFTView_Common::notifyMessagesRestored(void)
{
    MeshtasticView::notifyMessagesRestored();
    lv_obj_add_flag(THIS->objects.msg_restore_panel, LV_OBJ_FLAG_HIDDEN);
    updateActiveChats();
    updateNodesFiltered(true);
}

void TFTView_Common::showMessagePopup(uint32_t from, uint32_t to, uint8_t ch, const char *name)
{
    if (name) {
        static char buf[64];
        sprintf(buf, _("New message from \n%s"), name);
        buf[38] = '\0'; // cut too long userName
        lv_label_set_text(THIS->objects.msg_popup_label, buf);
        if (to == UINT32_MAX)
            THIS->objects.msg_popup_button->user_data = (void *)(uint32_t)ch; // store the channel in the button's data
        else
            THIS->objects.msg_popup_button->user_data = (void *)from; // store the node in the button's data
        lv_obj_clear_flag(THIS->objects.msg_popup_panel, LV_OBJ_FLAG_HIDDEN);
...
void TFTView_Common::hideMessagePopup(void)
{
    lv_obj_add_flag(THIS->objects.msg_popup_panel, LV_OBJ_FLAG_HIDDEN);
}

void TFTView_Common::updateUnreadMessages(void)
{
    char buf[64];
    if (THIS->unreadMessages > 0) {
        sprintf(buf, THIS->unreadMessages == 1 ? _("%d new message") : _("%d new messages"), THIS->unreadMessages);
        lv_obj_set_style_bg_img_src(THIS->objects.home_mail_button, &img_home_mail_unread_button_image,
                                    LV_PART_MAIN | LV_STATE_DEFAULT);
    } else {
        strcpy(buf, _("no new messages"));
        lv_obj_set_style_bg_img_src(THIS->objects.home_mail_button, &img_home_mail_button_image, LV_PART_MAIN | LV_STATE_DEFAULT);
    }
    lv_label_set_text(THIS->objects.home_mail_label, buf);

    // virtual hook for view-specific badge updates
    THIS->onUnreadMessagesUpdated();
}

// ===== UI Event Handlers (extracted from view files) =====

void TFTView_Common::ui_event_AlertButton(lv_event_t *e)
{
    lv_obj_add_flag(objects.alert_panel, LV_OBJ_FLAG_HIDDEN);
}

void TFTView_Common::ui_event_BellButton(lv_event_t *e)
{
...
void TFTView_Common::ui_event_BlankScreenButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_BluetoothButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_CancelQrButton(lv_event_t *e)
{
...
void TFTView_Common::ui_event_ChannelButton(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_EnvelopeButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
        if (!THIS->messagesRestored) {
...
void TFTView_Common::ui_event_GroupsButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_Keyboard(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_KeyboardButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_LoRaButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_LocationButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_LogoButton(lv_event_t *e)
{
...
void TFTView_Common::ui_event_MQTTButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_MapButton(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_MemoryButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_MessagesButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
        if (THIS->messagesRestored) {
...
void TFTView_Common::ui_event_MsgPopupButton(lv_event_t *e)
{
...
void TFTView_Common::ui_event_MsgRestoreButton(lv_event_t *e)
{
    lv_obj_add_flag(objects.msg_restore_panel, LV_OBJ_FLAG_HIDDEN);
}

void TFTView_Common::ui_event_NodesButton(lv_event_t *e)
{
    static bool ignoreClicked = false;
...
void TFTView_Common::ui_event_OnlineNodesButton(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_QrButton(lv_event_t *e)
{
...
void TFTView_Common::ui_event_SDCardButton(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_SettingsButton(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_TimeButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_WLANButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_alert_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_arrow(lv_event_t *e)
{
...
void TFTView_Common::ui_event_backup_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_backup_restore_radio_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_brightness_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
        char buf[20];
...
void TFTView_Common::ui_event_brightness_slider(lv_event_t *e)
{
...
void TFTView_Common::ui_event_calibration_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_calibration_screen_loaded(lv_event_t *e)
{
...
void TFTView_Common::ui_event_cancel(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
        switch (THIS->activeSettings) {
        case TFTView_Common::eSetup: {
        ...
        case TFTView_Common::eScreenTimeout: {
        ...
        case TFTView_Common::eScreenBrightness: {
        ...
        case TFTView_Common::eAlertBuzzer: {
        ...
        case TFTView_Common::eBackupRestore: {
...
void TFTView_Common::ui_event_channel_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_delete_channel(lv_event_t *e)
{
...
void TFTView_Common::ui_event_device_cancel_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_device_progmode_button(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_device_reboot_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_device_shutdown_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_frequency_slot_slider(lv_event_t *e)
{
    lv_obj_t *slider = lv_event_get_target_obj(e);
    char buf[40];
...
void TFTView_Common::ui_event_generate_psk(lv_event_t *e)
{
...
void TFTView_Common::ui_event_input_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_language_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_lockGps(lv_event_t *e)
{
...
void TFTView_Common::ui_event_mapBrightnessSlider(lv_event_t *e)
{
...
void TFTView_Common::ui_event_mapContrastSlider(lv_event_t *e)
{
...
void TFTView_Common::ui_event_mapNodeButton(lv_event_t *e)
{
...
void TFTView_Common::ui_event_mesh_detector(lv_event_t *e)
{
    THIS->ui_set_active(objects.settings_button, objects.mesh_detector_panel, objects.top_mesh_detector_panel);
}

void TFTView_Common::ui_event_mesh_detector_start(_lv_event_t *e)
{
...
void TFTView_Common::ui_event_modem_preset_dropdown(lv_event_t *e)
{
...
void TFTView_Common::ui_event_modify_channel(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_navHome(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_node_details(lv_event_t *e)
{
    THIS->ui_set_active(objects.settings_button, objects.details_panel, objects.top_nodes_panel);
}

void TFTView_Common::ui_event_ok(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
        switch (THIS->activeSettings) {
...
void TFTView_Common::ui_event_packet_log(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_pin_screen_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_positionButton(lv_event_t *e)
{
...
void TFTView_Common::ui_event_preset_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_qr_code(lv_event_t *e)
{
...
void TFTView_Common::ui_event_reboot_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
        lv_obj_remove_flag(objects.boot_logo_button, LV_OBJ_FLAG_HIDDEN);
...
void TFTView_Common::ui_event_region_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_reset_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_role_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_screen_lock_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
        char buf[10];
...
void TFTView_Common::ui_event_screen_timeout_slider(lv_event_t *e)
{
    lv_obj_t *slider = lv_event_get_target_obj(e);
    char buf[20];
...
void TFTView_Common::ui_event_setup_region_dropdown(lv_event_t *e) {}

void TFTView_Common::ui_event_signal_scanner(lv_event_t *e)
{
    if (currentPanel) {
...
void TFTView_Common::ui_event_signal_scanner_node(lv_event_t *e)
{
...
void TFTView_Common::ui_event_signal_scanner_start(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (currentNode) {
        static bool ignoreClicked = false;
...
void TFTView_Common::ui_event_statistics(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
...
void TFTView_Common::ui_event_statistics_table(lv_event_t *e)
{
...
void TFTView_Common::ui_event_theme_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_timeout_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_trace_route(lv_event_t *e)
{
...
void TFTView_Common::ui_event_trace_route_node(lv_event_t *e)
{
    // navigate to node in node list
...
void TFTView_Common::ui_event_trace_route_start(lv_event_t *e)
{
    if (!spinnerButton) {
        if (currentPanel) {
            time(&startTime);
...
void TFTView_Common::ui_event_trace_route_to(lv_event_t *e)
{
    if (!spinnerButton) {
...
void TFTView_Common::ui_event_user_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
...
void TFTView_Common::ui_event_wifi_button(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
void TFTView_Common::ui_event_zoomIn(lv_event_t *e)
{
    THIS->map->setZoom(MapTileSettings::getZoomLevel() + 1);
    THIS->updateLocationMap(THIS->map->getObjectsOnMap());
}

void TFTView_Common::ui_event_zoomOut(lv_event_t *e)
{
    THIS->map->setZoom(MapTileSettings::getZoomLevel() - 1);
    THIS->updateLocationMap(THIS->map->getObjectsOnMap());
}

void TFTView_Common::ui_event_zoomSlider(lv_event_t *e)
{
    THIS->map->setZoom(lv_slider_get_value(objects.zoom_slider));
    THIS->updateLocationMap(THIS->map->getObjectsOnMap());
}
void TFTView_Common::init(IClientBase *client)
{
    ILOG_DEBUG("TFTView_Common init...");
    ILOG_DEBUG("TFTView_Common db size: %d", sizeof(TFTView_Common));
 ...
 */
bool TFTView_Common::setupUIConfig(const meshtastic_DeviceUIConfig &uiconfig)
{
 ...
 */
void TFTView_Common::updateBootMessage(const char *msg)
{
    if (msg)
 ...
 *
 */
void TFTView_Common::init_screens(void)
{
    ...
    // view-specific screen initialization (e.g. messagesBadge for 480x222)
    onInitScreensExtra();

    screensInitialised = true;
    state = MeshtasticView::eInitDone;
    ILOG_DEBUG("TFTView_Common init done.");
}
 ...
 */
void TFTView_Common::enterProgrammingMode(void)
{
 ...
 *
 */
void TFTView_Common::apply_hotfix(void)
{
...
void TFTView_Common::updateTheme(void)
{
    Themes::initStyles();
...
void TFTView_Common::ui_events_init(void)
{
    ...
    // view-specific event registrations (GlobalKeyHandler, MainButtonFocus, etc.)
    onEventsInitExtra();
}
...
void TFTView_Common::timer_event_programming_mode(lv_timer_t *timer)
{
...
void TFTView_Common::ui_event_NodeButton(lv_event_t *e)
{
...
void TFTView_Common::ui_event_ChatButton(lv_event_t *e)
{
    static bool ignoreClicked = false;
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target_obj(e);
 ...
 * @param e
 */
void TFTView_Common::ui_event_ChatDelButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED) {
        lv_obj_t *target = lv_event_get_target_obj(e);
 ...
 */
void TFTView_Common::ui_event_message_ready(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
...
// configuration reset
// reboot / shutdown
void TFTView_Common::ui_event_chatNodeButton(lv_event_t *e)
{
    uint32_t nodeNum = (unsigned long)e->user_data;
    auto it = THIS->nodes.find(nodeNum);
    if (it != THIS->nodes.end()) {
...
void TFTView_Common::ui_screen_event_cb(lv_event_t *e)
{
    if (THIS->activePanel == objects.map_panel) {
...
void TFTView_Common::updateLocationMap(uint32_t num)
{
 ...
 */
void TFTView_Common::addOrUpdateMap(uint32_t nodeNum, int32_t lat, int32_t lon)
{
...
void TFTView_Common::removeFromMap(uint32_t nodeNum)
{
    auto it = nodeObjects.find(nodeNum);
    if (it == nodeObjects.end())
        return;
...
void TFTView_Common::removeSpinner(void)
{
    if (spinnerButton) {
        lv_obj_delete(spinnerButton);
        spinnerButton = nullptr;
...
void TFTView_Common::packetDetected(const meshtastic_MeshPacket &p)
{
    uint32_t heard = 0;
    if (p.from != ownNode)
        heard = p.from;
...
void TFTView_Common::requestSetup(void)
{
 ...
 */
void TFTView_Common::updateSignalStrength(int32_t rssi, float snr)
{
 ...
 */
void TFTView_Common::setLocale(meshtastic_Language lang)
{
 ...
 */
void TFTView_Common::setLanguage(meshtastic_Language lang)
{
 ...
 * @brief Set timeout
 */
void TFTView_Common::setTimeout(uint32_t timeout)
{
    char buf[32];
    if (timeout == 0)
 ...
 */
void TFTView_Common::setBrightness(uint32_t brightness)
{
    char buf[32];
 ...
 */
void TFTView_Common::setTheme(uint32_t value)
{
 ...
 */
void TFTView_Common::storeNodeOptions(void)
{
 ...
 */
void TFTView_Common::eraseChat(uint32_t channelOrNode)
{
 ...
 */
void TFTView_Common::clearChatHistory(void)
{
    for (auto &it : chats) {
        lv_obj_delete(it.second);
...
// animations
void TFTView_Common::ui_anim_node_panel_cb(void *var, int32_t v)
{
    lv_obj_set_height((lv_obj_t *)var, v);
}

void TFTView_Common::ui_anim_radar_cb(void *var, int32_t r)
{
 ...
 * @param func
 */
void TFTView_Common::showUserWidget(UserWidgetFunc createWidget)
{
...
void TFTView_Common::handleAddMessage(char *msg)
{
 ...
 */
void TFTView_Common::addMessage(lv_obj_t *container, uint32_t msgTime, uint32_t requestId, char *msg,
...
void TFTView_Common::setMyInfo(uint32_t nodeNum)
{
    ownNode = nodeNum;
}

void TFTView_Common::setDeviceMetaData(int hw_model, const char *version, bool has_bluetooth, bool has_wifi, bool has_eth,
                                        bool can_shutdown)
{
}
void TFTView_Common::updatePosition(uint32_t nodeNum, int32_t lat, int32_t lon, int32_t alt, uint32_t sats, uint32_t precision)
{
...
void TFTView_Common::updateDistance(uint32_t nodeNum, int32_t lat, int32_t lon)
{
 ...
 * @param airUtil
 */
void TFTView_Common::updateMetrics(uint32_t nodeNum, uint32_t bat_level, float voltage, float chUtil, float airUtil)
{
...
void TFTView_Common::updateEnvironmentMetrics(uint32_t nodeNum, const meshtastic_EnvironmentMetrics &metrics)
{
    auto it = nodes.find(nodeNum);
    if (it != nodes.end()) {
        char buf[50];
...
void TFTView_Common::updateAirQualityMetrics(uint32_t nodeNum, const meshtastic_AirQualityMetrics &metrics)
{
    auto it = nodes.find(nodeNum);
...
void TFTView_Common::updatePowerMetrics(uint32_t nodeNum, const meshtastic_PowerMetrics &metrics)
{
    auto it = nodes.find(nodeNum);
    if (it != nodes.end() && it->first != ownNode) {
        // TODO
 ...
 */
void TFTView_Common::updateSignalStrength(uint32_t nodeNum, int32_t rssi, float snr)
{
    if (nodeNum != ownNode) {
        auto it = nodes.find(nodeNum);
        if (it != nodes.end()) {
...
void TFTView_Common::updateHopsAway(uint32_t nodeNum, uint8_t hopsAway)
{
    if (nodeNum != ownNode) {
        auto it = nodes.find(nodeNum);
        if (it != nodes.end()) {
...
void TFTView_Common::updateConnectionStatus(const meshtastic_DeviceConnectionStatus &status)
{
...
void TFTView_Common::onTextMessageCallback(const ResponseHandler::Request &req, ResponseHandler::EventType evt, int32_t result)
{
...
void TFTView_Common::onPositionCallback(const ResponseHandler::Request &req, ResponseHandler::EventType evt, int32_t) {}

void TFTView_Common::onTracerouteCallback(const ResponseHandler::Request &req, ResponseHandler::EventType evt, int32_t) {}

/**
 * handle response from routing
 */
void TFTView_Common::handleResponse(uint32_t from, const uint32_t id, const meshtastic_Routing &routing,
 ...
 * Signal scanner
 */
void TFTView_Common::scanSignal(uint32_t scanNo)
{
...
void TFTView_Common::handlePositionResponse(uint32_t from, uint32_t request_id, int32_t rx_rssi, float rx_snr, bool isNeighbor)
{
 ...
 */
void TFTView_Common::handleTraceRouteResponse(const meshtastic_Routing &routing)
{
...
void TFTView_Common::handleResponse(uint32_t from, uint32_t id, const meshtastic_RouteDiscovery &route)
{
 ...
 * @param ack
 */
void TFTView_Common::handleTextMessageResponse(uint32_t channelOrNode, const uint32_t id, bool ack, bool err)
{
...
void TFTView_Common::packetReceived(const meshtastic_MeshPacket &p)
{
...
void TFTView_Common::notifyConnected(const char *info)
{
...
void TFTView_Common::notifyDisconnected(const char *info)
{
    if (state == MeshtasticView::eBooting) {
        updateBootMessage(info);
    } else {
...
void TFTView_Common::notifyResync(bool show)
{
    if (controller->isStandalone()) {
        if (show)
            notifyReboot(true);
...
void TFTView_Common::notifyReboot(bool show)
{
...
void TFTView_Common::notifyShutdown(void)
{
    messageAlert(_("Shutting down ..."), true);
}

void TFTView_Common::blankScreen(bool enable)
{
...
void TFTView_Common::screenSaving(bool enabled)
{
    if (enabled) {
...
bool TFTView_Common::isScreenLocked(void)
{
    return screenLocked && !screenUnlockRequest;
}

void TFTView_Common::updateChannelConfig(const meshtastic_Channel &ch)
{
    ...
    if (ch.role != meshtastic_Channel_Role_DISABLED) {
        ...
        lv_obj_set_width(btn[ch.index], lv_pct(getChannelButtonWidth()));
...
void TFTView_Common::updateGroupChannel(uint8_t chId)
{
    static lv_obj_t *btn[c_max_channels] = {objects.channel_button0, objects.channel_button1, objects.channel_button2,
                                            objects.channel_button3, objects.channel_button4, objects.channel_button5,
                                            objects.channel_button6, objects.channel_button7};
...
void TFTView_Common::updateDeviceConfig(const meshtastic_Config_DeviceConfig &cfg)
{
...
void TFTView_Common::updatePositionConfig(const meshtastic_Config_PositionConfig &cfg)
{
...
void TFTView_Common::updatePowerConfig(const meshtastic_Config_PowerConfig &cfg)
{
    db.config.power = cfg;
    db.config.has_power = true;
}

void TFTView_Common::updateNetworkConfig(const meshtastic_Config_NetworkConfig &cfg)
{
...
void TFTView_Common::updateDisplayConfig(const meshtastic_Config_DisplayConfig &cfg)
{
...
void TFTView_Common::updateLoRaConfig(const meshtastic_Config_LoRaConfig &cfg)
{
    db.config.lora = cfg;
...
void TFTView_Common::showLoRaFrequency(const meshtastic_Config_LoRaConfig &cfg)
{
    char loraFreq[48];
    if (!cfg.region) {
...
void TFTView_Common::setBellText(bool banner, bool sound)
{
    if (banner && sound) {
 ...
 */
void TFTView_Common::setChannelName(const meshtastic_Channel &ch)
{
    char buf[40];
    if (ch.role == meshtastic_Channel_Role_PRIMARY) {
...
void TFTView_Common::backup(uint32_t option)
{
#if defined(HAS_SDCARD) || defined(HAS_SD_MMC) || defined(ARCH_PORTDUINO)
    if (option == 1) {
        backupFullConfig();
...
void TFTView_Common::restore(uint32_t option)
{
#if defined(HAS_SDCARD) || defined(HAS_SD_MMC) || defined(ARCH_PORTDUINO)
    if (option == 1) {
        restoreFullConfig();
 ...
 */
uint32_t TFTView_Common::timestamp(char *buf, uint32_t datetime, bool update)
{
 ...
 */
int32_t TFTView_Common::signalStrength2Percent(int32_t rx_rssi, float rx_snr)
{
#if defined(USE_SX127x)
...
void TFTView_Common::updateBluetoothConfig(const meshtastic_Config_BluetoothConfig &cfg, uint32_t id)
{
...
void TFTView_Common::updateSecurityConfig(const meshtastic_Config_SecurityConfig &cfg)
{
...
void TFTView_Common::updateSessionKeyConfig(const meshtastic_Config_SessionkeyConfig &cfg)
{
    // TODO
}
...
void TFTView_Common::updateMQTTModule(const meshtastic_ModuleConfig_MQTTConfig &cfg)
{
...
void TFTView_Common::updateExtNotificationModule(const meshtastic_ModuleConfig_ExternalNotificationConfig &cfg)
{
...
void TFTView_Common::updateRingtone(const char rtttl[231])
{
...
void TFTView_Common::updateTime(uint32_t timeVal)
{
    time_t localtime;
    time(&localtime);
 ...
 * @param textArea
 */
void TFTView_Common::showKeyboard(lv_obj_t *textArea)
{
...
void TFTView_Common::hideKeyboard(lv_obj_t *panel)
{
    lv_area_t kb_coords;
    lv_obj_get_coords(objects.keyboard, &kb_coords);
    uint32_t kb_h = kb_coords.y2 - kb_coords.y1;
...
lv_obj_t *TFTView_Common::showQrCode(lv_obj_t *parent, const char *data)
{
 ...
 */
void TFTView_Common::enablePanel(lv_obj_t *panel)
{
 ...
 */
void TFTView_Common::disablePanel(lv_obj_t *panel)
{
 ...
 */
void TFTView_Common::setGroupFocus(lv_obj_t *panel)
{
 ...
 */
void TFTView_Common::setInputGroup(void)
{
    lv_group_t *group = lv_group_get_default();
...
void TFTView_Common::setInputButtonLabel(void)
{
 ...
 *
 */
void TFTView_Common::updateTime(void)
{
    char buf[80];
    time_t curr_time;
#ifdef ARCH_PORTDUINO
...
bool TFTView_Common::updateSDCard(void)
{
    formatSD = false;
    if (sdCard) {
        delete sdCard;
...
void TFTView_Common::formatSDCard(void)
{
    if (sdCard) {
        delete sdCard;
        sdCard = nullptr;
...
void TFTView_Common::updateFreeMem(void)
{
...
void TFTView_Common::task_handler(void)
{
